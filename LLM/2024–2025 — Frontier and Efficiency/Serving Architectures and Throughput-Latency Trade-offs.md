...
For a practical local experiment, use [[LLM/Study/Local LLM Concurrency and Batch Throughput Lab|Local LLM Concurrency and Batch Throughput Lab]] to compare single-request latency against a controlled concurrency ladder. Use [[LLM/Study/Local LLM Observability and Operations Runbook|Local LLM Observability and Operations Runbook]] when a latency or throughput claim needs request logs, server metrics, queue/KV/cache state, and resource-pressure evidence.
