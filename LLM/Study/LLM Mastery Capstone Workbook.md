...
| Model-store bootstrap runner | Results JSON/CSV/Markdown from [[LLM/Study/Local LLM Model Store Bootstrap Runner|Local LLM Model Store Bootstrap Runner]] |  | The run folder, model-store directories, and user cache variables are planned or applied with pass/hold/fail evidence before runtime install or model pull. | Not started |
...
| First runtime health runner | Results JSON/CSV/Markdown from [[LLM/Study/Local LLM First Runtime Health Runner|Local LLM First Runtime Health Runner]] |  | Listener, native API, installed model ids, running-model or idle state, OpenAI-compatible model ids, expected model visibility, loopback boundary, pass/hold/fail decision, and next route are audited before the first prompt. | Not started |
| First smoke request runner | Summary JSON/Markdown from [[LLM/Study/Local LLM First Smoke Request Runner|Local LLM First Smoke Request Runner]] |  | Runtime-health JSON, model id, native and OpenAI-compatible base URLs, request, response, output, route decision, missing layer, and next action are captured for the first controlled local inference prompt. | Not started |
