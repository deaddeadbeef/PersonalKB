...
| C2/C4 fails only after a lower-bit or offload change | Weight memory improved but KV cache, shared memory, or quality/latency regressed under load. | Run [[LLM/Study/Local LLM Quantization and GPU Offload Lab|Local LLM Quantization and GPU Offload Lab]] with the target concurrency setting. |
