...
When this endpoint will be used again after today, fill the [[LLM/Study/Local LLM Service Lifecycle and Upgrade Runbook|Local LLM Service Lifecycle and Upgrade Runbook]] Change Freeze Card before changing runtime version, model artifact, startup mode, cache path, UI container, driver, or client contract. Use [[LLM/Study/Local LLM Service Lifecycle and Upgrade Runner|Local LLM Service Lifecycle and Upgrade Runner]] when the freeze card and evidence files need a checked before/after/rollback package.
