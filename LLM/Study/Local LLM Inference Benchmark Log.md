...
For document-grounded workloads, use [[LLM/Study/Local Embedding and Reranker Hosting Lab|Local Embedding and Reranker Hosting Lab]], [[LLM/Study/Local RAG Assistant Lab|Local RAG Assistant Lab]], [[LLM/Study/Local RAG Retrieval Evaluation and Reranking Lab|Local RAG Retrieval Evaluation and Reranking Lab]], and [[LLM/Study/Local RAG Minimal Python Harness|Local RAG Minimal Python Harness]] to record embedding/reranker service cards, corpus version, chunk policy, embedding/index configuration, top-k hits, first relevant rank, reranking or hybrid decisions, retrieval results, citation support, unsupported-question refusal, and RAG-specific failure modes.
...
- If the benchmark depends on embeddings or reranking, save the service card from [[LLM/Study/Local Embedding and Reranker Hosting Lab|Local Embedding and Reranker Hosting Lab]] so vector dimension, normalization, batching, reranker candidate count, and latency are not hidden variables.
- If the RAG row has a weak or missing retrieval explanation, run [[LLM/Study/Local RAG Retrieval Evaluation and Reranking Lab|Local RAG Retrieval Evaluation and Reranking Lab]] before changing the generator model.
