...
```mermaid
flowchart LR
    subgraph SOV["Japanese SOV Sentence Order"]
        direction LR
        S["Subject は/が"] --> O["Object を"] --> V["Verb"]
    end
    subgraph Particles["Key Particle Functions"]
        direction TB
        WA["は — Topic"] ~~~ GA["が — Subject"]
        WO["を — Object"] ~~~ NI["に — Direction / Time"]
        DE["で — Location / Means"] ~~~ NO["の — Possession"]
    end
    style S fill:#e8f5e9
    style O fill:#e3f2fd
    style V fill:#fff3e0
