---
tags: [japanese, listening, phase-1, audio, coverage]
up: "[[Listening Overview]]"
confidence: policy
freshness: stable
tier-coverage: [core, practice]
---

# Phase 1 Audio Coverage Map

> Use this page when asking: "What should I listen to while studying this Phase 1 page?" It maps the required [[Phase 1 — Foundation]] sequence to local clips, authentic audio, and pronunciation QA.

## Coverage Rule

Every Phase 1 study item needs three things:

| Layer | Source | Job |
| --- | --- | --- |
| Local drill | [[Phase 1 Local Audio Practice]] or page-embedded MP3 clips | Repeat small sounds, words, and sentences inside Obsidian |
| Authentic model | [[Phase 1 Authentic Audio Spine]] | Keep human-recorded or official-course Japanese as the pronunciation model |
| QA rule | [[Pronunciation and Audio Accuracy]] | Decide when a clip is safe, suspect, or needs correction |

For the first week, use [[Phase 1 Audio Starter Assignment]] before choosing any extra resource. For Weeks 2-4, use [[Phase 1 Audio Assignment Ladder]].

Planning pages do not need embedded audio. They need to route clearly to the audio layers. Drill pages and content pages should either embed clips directly or point to the matching weekly clip set.

## Required Phase 1 Coverage

| Page | Embedded local clips | Authentic model | QA status | Use while studying |
| --- | ---: | --- | --- | --- |
| [[Japanese Learning Dashboard]] | 0 | Routes to spine | Routes to QA | Choose today's audio target |
| [[Phase 1 — Foundation]] | 0 | Routes to spine | Routes to QA | Follow the weekly audio pairing |
| [[Writing Systems Overview]] | 10 | Week 1-2 native course audio | Covered by QA | Hear the three-script system and kana examples |
| [[Hiragana Complete Guide]] | 102 | Week 1 native course audio | Covered by QA | Drill kana rows, particle は/へ, long vowels, and small っ |
| [[Katakana Complete Guide]] | 76 | Week 2 native course audio | Covered by QA | Drill loanword timing and katakana recognition |
| [[Core 100 — Survival Japanese]] | 71 | Week 3 native course or NHK phrases | Covered by QA | Pair survival words with local phrase repetition |
| [[N5 Grammar — Sentence Patterns]] | 30 | Week 3-4 course dialogue audio | Covered by QA | Hear XはYです, requests, and first sentence frames |
| [[Conversation Patterns — Greetings and Introductions]] | 28 | Week 4 course or NHK introduction audio | Covered by QA | Practice greetings before the self-introduction |
| [[Self-Introduction Template]] | 28 | Week 4 native/tutor/course model | Covered by QA | Build the memorized first script |
| [[Phase 1 Local Audio Practice]] | 41 | Must pair with spine | Explicit correction workflow | Daily 5-minute local drill |
| [[Phase 1 Authentic Audio Spine]] | 0 | Primary native/official route | Quality bar included | Choose Genki, Irodori, or NHK as the model |
| [[Phase 1 Audio Starter Assignment]] | 0 | First-week source assignment | Routes to QA and evidence log | Copy the first exact audio target into the evidence log |
| [[Phase 1 Audio Assignment Ladder]] | 0 | Weeks 1-4 assignment route | Routes to QA and evidence log | Keep each Phase 1 week tied to one exact audio target |
| [[Pronunciation and Audio Accuracy]] | 0 | Uses native/accent references | Primary QA page | Check suspect clips and correction decisions |
| [[Pronunciation Correction Log]] | 0 | Records corrections | Open/resolved QA log | Keep suspect clips out of daily practice |
| [[Beginner Listening Resources]] | 0 | Broader external resources | Routes to QA through the audio stack | Use after the Phase 1 defaults are clear |
| [[Phase 1 Weekly Review]] | 0 | Logs native target | Logs local target and QA check | Record evidence and next audio target |

## Weekly Audio Pairing

| Week | Study pages | Local drill | Authentic model | Proof |
| --- | --- | --- | --- | --- |
| 1 | [[Hiragana Complete Guide]], [[Writing Systems Overview]] | Week 1 of [[Phase 1 Local Audio Practice]] | First Genki/Irodori/NHK lesson audio | Hear and produce vowels, は/へ, を, long vowels, and small っ |
| 2 | [[Katakana Complete Guide]], [[Writing Systems Overview]] | Week 2 of [[Phase 1 Local Audio Practice]] | Next course or NHK beginner audio | Say コーヒー, タクシー, ホテル with Japanese timing |
| 3 | [[Core 100 — Survival Japanese]], [[N5 Grammar — Sentence Patterns]] | Week 3 of [[Phase 1 Local Audio Practice]] | First short dialogue/request audio | Shadow five survival phrases |
| 4 | [[Conversation Patterns — Greetings and Introductions]], [[Self-Introduction Template]] | Week 4 of [[Phase 1 Local Audio Practice]] | Self-introduction or greeting dialogue audio | Say a 20-30 second self-introduction from memory |

## Gap Policy

These are not gaps:

- A planning page with zero embedded clips, if it routes to the audio stack.
- A copyrighted native source linked externally instead of mirrored into the vault.
- A local TTS clip used only after [[Pronunciation and Audio Accuracy]] says it is safe for the job.

These are real gaps:

- A required Phase 1 page with no embedded clips and no route to local/authentic audio.
- A local clip in a daily path that sounds wrong or lacks a reading hint for ambiguous text.
- A learner-facing instruction that says "listen to audio" without naming the page or source.
- A Phase 1 week with no [[Phase 1 Weekly Review]] evidence for the native segment, local clip set, and pronunciation or clip QA check.

## Next Coverage Targets

- Use [[Pronunciation Correction Log]] when a real suspect clip is found.
- Use [[Phase 2 Audio Coverage Map]] and [[Phase 2 Weekly Review]] before making Phase 2 the active dashboard phase.
- Keep the local audio ladder small; expand only when the weekly proof needs a missing clip.

## References

- [[Phase 1 — Foundation]]
- [[Phase 1 Audio Starter Assignment]]
- [[Phase 1 Audio Assignment Ladder]]
- [[Phase 1 Local Audio Practice]]
- [[Phase 1 Authentic Audio Spine]]
- [[Phase 1 Weekly Review]]
- [[Pronunciation and Audio Accuracy]]
- [[Pronunciation Correction Log]]
- [[Beginner Listening Resources]]
- [[Phase 2 Audio Coverage Map]]
- [[Phase 2 Weekly Review]]
- [[Japanese/Sources/Sources Index|Sources Index]]
