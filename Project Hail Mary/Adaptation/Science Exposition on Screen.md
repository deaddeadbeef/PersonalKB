...
```mermaid
flowchart LR
    A[Novel Science Reasoning] --> B{Screen Exposition}
    B --> C[Internal Monologue → Dialogue & Logs]
    B --> D[Iterative Testing → Montage]
    B --> E[Technical Graphs → Visual Metaphor]
    B --> F[Detailed Mechanism → Selective Omission]
    B --> G[Earth-side Discovery → Flashbacks]
```
