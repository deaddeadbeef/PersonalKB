...
```mermaid
flowchart LR
    A[Novel Structure] --> B{Film Adaptation}
    B --> C[Internal Monologue → Spoken Logs]
    B --> D[Iterative Experimentation → Montages]
    B --> E[Slow Language Building → Accelerated Dialogue]
    B --> F[Earth-side Subplots → Simplified]
    B --> G[Detailed Lab Work → Visual Summary]
```
