...
```mermaid
flowchart TD
    A[Mirrored signals establish intent]
    B[Orrery models reveal home systems]
    C[Tunnel contact reveals environment and biology]
    D[Objects, clocks, and yes/no signals build vocabulary]
    E[Shared language enables joint science]
    F[Epilogue shows mature cultural exchange]
    A --> B --> C --> D --> E --> F
```
