...
```mermaid
flowchart TD
    A[Tau Ceti anomaly suggests suppression]
    B[Adrian sampling retrieves biological evidence]
    C[Taumoeba is observed and named]
    D[Nitrogen is isolated as the killer]
    E[Taumoeba-82.5 is bred for deployment]
    F[Xenonite permeability reveals the cost]
    A --> B --> C --> D --> E --> F
```
