...
```mermaid
flowchart TD
    A[Solar dimming noticed] --> B[Petrova detects absorption feature]
    B --> C[Spectral analysis isolates Petrova Line]
    C --> D[Known physical cause?]
    D -->|No| E[Investigate novel absorber]
    E --> F[Astrophage hypothesis]
    F --> G[Global crisis response]
```
