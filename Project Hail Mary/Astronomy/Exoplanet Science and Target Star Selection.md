...
```mermaid
flowchart TD
    A[Nearby stars surveyed] --> B[Most systems show dimming]
    B --> C[Tau Ceti appears stable]
    C --> D[Hail Mary targets Tau Ceti]
    D --> E[Investigate Astrophage ecology]
    E --> F[Rocky reveals 40 Eridani origin]
    F --> G[Two real star systems anchor the plot]
```
